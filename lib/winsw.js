module.exports = {
  ...
  createExe: function(name,dir,callback){
    ...
    var origin = p.join(__dirname,'..','bin','winsw','x'+(require('os').arch().indexOf('64')>0 ? '64':'86'),'winsw.exe'),
        originCfg = p.join(__dirname,'..','bin','winsw','x'+(require('os').arch().indexOf('64')>0 ? '64':'86'),'winsw.exe.config'),
        dest = p.join(dir,name.replace(/[^\w]/gi,'').toLowerCase()+'.exe'),
        destCfg = p.join(dir,name.replace(/[^\w]/gi,'').toLowerCase()+'.exe.config'),
        data = fs.readFileSync(origin,{encoding:'binary'}),
        dataCfg = fs.readFileSync(originCfg,{encoding:'binary'});

    fs.writeFileSync(dest,data,{encoding:'binary'});
    fs.writeFileSync(destCfg,dataCfg,{encoding:'binary'});
    callback && callback();
